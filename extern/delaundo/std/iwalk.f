      Integer FUNCTION IWALK (X,Y,NVvr,XNode,YNode,NFrmNde,NghVvr,MVvr,
     &                        FOutCnv)
C
C Last update:
C 20Aug93,16:25; intro IFFYWALK.
C 20Nov92,14:15; derived from WALK.
C
...
C and the outward normal on that face.
C
C Input:
...
C
C     Since the coming to matter of NDEX, we have to deal with the
C     case of a formerly physical cell, that has been erased, 
C     invalidated, removed whatever.
      NVvr = MIN(NVvr,MVvr)
C
C     Check for physicality of the cell.
...
C       or on the triangle. Now check more precisely with scaled
C       normals.
        IWalk = IFFYWALK(X,Y,NVvr,XNode,YNode,NFrmNde,NghVvr,MVvr,
     &                      FOutCnv)
        RETURN
C
      END IF
...
C         For aesthetics, give a zero IWALK to avoid confusing
C         compiler warnings.
          IWALK = 0
          FOutCnv = .TRUE.
          RETURN
...
C
      WRITE (*,'(2A/A,2E14.6)') 
     &               ' FATAL: IWALK caught in an endless loop in',
     &               ' search for',' X,Y=',X,Y
      STOP
C
      END
